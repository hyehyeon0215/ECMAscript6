...
//___________________________// 활성화 클래스를 이용한 제어
...
for (let i = 0; i < btns.length; i++) {
    ...
    btns[i].addEventListener("click", function () {
        ...
        /*on(활성화클래스)가 0인덱스에 있는데 js는 어느 인덱스에 있는지를
        추적해야 알수있습니다. 하지만 그 추적하는 코드가 길고 복잡합니다
        따라서 js코드를 적게 써야하기 때문에 우리는 on클래스를 추적해서
        on클래스만 제거하는 것이아니라 on클래스를 모든 곳에서 제거하고
        그후에 필요한 곳에 on클래스를 부여하는 방법을 사용합니다 */
        //on활성화 클래스는 두군데에 존재합니다
...
함수 패키징 - 자주사용되거나, 같은 기능을 하는 내용을 함수로 패키징 
합니다. 이때 함수에 매개변수를 만들어서 틀을 만들어주는것입니다
장점 : 코드가 비교적 짧아지고, 재활용을 하기때문에 생산성이 높아집니다
단점 : 함수가 너무 많아집니다, 함수안에 또 함수가 오는등의 복잡해집니다
=> 결과 생산성이 더 떨어지는경우가 발생합니다

객체지향 - 함수패키징이 눈오리도구였다면 객체지향은 공장입니다
클래스를 만들어서 생산합니다
단점? 없습니다 어려워요
